...
function generateWhereClauseAndParams(filters, entityAlias, existingParams = {}) {
    let whereClauses = [];
    let params = { ...existingParams };
    for (const columnName in filters) {
        if (filters.hasOwnProperty(columnName)) {
            if (columnName === 'or')
                continue; // Skip 'or' here, it's handled separately at a higher level
            const filter = filters[columnName]; // We know column name is keyof T and Filter is the type
            if (filter) {
                if (filter.equal !== undefined) {
                    const paramName = `${columnName}_equal`;
                    whereClauses.push(`${entityAlias}.${columnName} = :${paramName}`);
                    params[paramName] = filter.equal;
                }
                else if (filter.notEqual !== undefined) {
                    const paramName = `${columnName}_notEqual`;
                    whereClauses.push(`${entityAlias}.${columnName} != :${paramName}`);
                    params[paramName] = filter.notEqual;
                }
                else if (filter.greaterThan !== undefined) {
                    const paramName = `${columnName}_greaterThan`;
                    whereClauses.push(`${entityAlias}.${columnName} > :${paramName}`);
                    params[paramName] = filter.greaterThan;
                }
                else if (filter.lessThan !== undefined) {
                    const paramName = `${columnName}_lessThan`;
                    whereClauses.push(`${entityAlias}.${columnName} < :${paramName}`);
                    params[paramName] = filter.lessThan;
                }
                else if (filter.betweenRange !== undefined) {
                    const [start, end] = filter.betweenRange.sort((a, b) => a - b); // Ensure ascending order
                ...
                else if (filter.betweenDate !== undefined) {
                    const [startDate, endDate] = filter.betweenDate.sort((a, b) => a.getTime() - b.getTime()); // Ensure older to newer
                    ...
                    params[paramName] = filter.in;
                }
                else if (filter.equalNull === true) {
                    whereClauses.push(`${entityAlias}.${columnName} IS NULL`);
                }
                else if (filter.equalNotNull === true) {
                    ...
                    params[paramName] = filter.notLike;
                }
            }
        }
    }
...
function applyFilters(// Add constraint 'extends ObjectLiteral' to T
queryBuilder, filters, entityAlias) {
    if (filters) {
        let combinedOrWhereClause = '';
        let combinedOrParams = {};
