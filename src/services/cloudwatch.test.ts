// src/services/cloudwatch.test.ts

import "../tests/setup"; // Import the setup file
...
import CloudWatchService from "./cloudWatch.service"; // Adjust path as needed
...
describe("CloudWatchService", () => {
    ...
    describe("logError", () => {
        ...
        it("should only log a message if a sequence token already exists", async () => {
            ...
            logsMock.on(CreateLogStreamCommand).resolves({});
            logsMock.on(PutLogEventsCommand).resolvesOnce({ nextSequenceToken: "first-token" });
            await service.logError("first message");

            // Second call
            logsMock.on(PutLogEventsCommand).resolvesOnce({ nextSequenceToken: "second-token" });
